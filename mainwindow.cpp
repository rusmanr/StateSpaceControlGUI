...
void MainWindow::mySwitch(const double& var){
    fmod(var, period) < dutyRatio*period ? (switchState = true) : (switchState = false);
}

double MainWindow::dVc(const double& Vc, const double& Il){
    if(switchState){
        return (resistance*Il - Vc)/(resistance*capacitance);
    }
    else{ //check ==========================================
        return (resistance*Il)/(resistance*capacitance);
//        return (resistance*Il - Vc)/(resistance*capacitance);
    }
}

double MainWindow::dIl(const double& Vc, const double& Vin){
    if(switchState){
        return (Vin - Vc)/inductance;
    }
    else{
        return (0.0 - Vc)/inductance;
    }
}
...
//        il.push_back(il[i] + rk4(vc[i], vIn, il[i], false));
        sw.push_back(now);
        il.push_back(now);
        vc.push_back(xcurrent[i]);

//        if(now >= samplingStart){sumOutput += (0.5*(vc[i+1]+v[i])*deltat);}
    }
...
//    ui->customPlot->graph(0)->addData(0, vc);
...
void MainWindow::on_plotButton_clicked()
{
    ...
    for(int i = 0; i < ITER_MAX; i++){
        ...
        xtime.push_back(now);
        model.Calculate();
        std::cout<<"i= "<< i <<"--" << model.getRealOutput()<<'\n';
        Eigen::MatrixXf temp = model.getRealOutput();
        xcurrent.push_back( temp.coeff(0,0));
    }
    subconstructor();
